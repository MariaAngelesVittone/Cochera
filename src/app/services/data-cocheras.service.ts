...
export class DataCocherasService {
  ...
  async agregarCochera(value: any) {
  ...
  deshabilitarCochera(id: number) {
    if (!this.cocheras || this.cocheras.length === 0) {
      console.error('El listado de cocheras no ha sido cargado.');
      return;
    }
    const cochera = this.cocheras.find(c => c.id === id);
    if (!cochera) {
      console.error(`No se encontró una cochera con el ID ${id}.`);
      return;
    }
    cochera.deshabilitada = 1;
  }
  
  habilitarCochera(id: number) {
    if (!this.cocheras || this.cocheras.length === 0) {
      console.error('El listado de cocheras no ha sido cargado.');
      return;
    }
